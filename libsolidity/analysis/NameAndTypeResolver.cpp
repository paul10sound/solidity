...
)
{
	...
	{
		// We allow exception for public functions and events
		if (
			auto functionDefinition = dynamic_cast<FunctionDefinition const*>(&_declaration);
			functionDefinition && !functionDefinition->isFree() && functionDefinition->isPublic()
		)
		{
		}
